...
// **記号まで見る**（`I-20260830-19`）。仮名と漢字だけを見ていたので、
// 読点 `、`(U+3001) と全角括弧 `（）`(U+FF08/FF09) が網から漏れ、
// 英語やアラビア語の画面へそのまま出ていた（実測11箇所＋5箇所）。
// U+3000–303F は CJK の約物、U+FF00–FFEF は全角形。
// **約物だけ足す**（`I-20260830-19`）。仮名と漢字しか見ていなかったので、
// 読点 `、`(U+3001) と全角括弧 `（）`(U+FF08/FF09) が網から漏れ、英語や
// アラビア語の画面へそのまま出ていた（実測11箇所＋5箇所）。
//
// **全角形の塊（U+FF00–FFEF）ごと足さない。** `＋` のように**記号として**
// 使っている字まで拾い、訳す物ではないものを訳せと言い出す（一度そうした）。
